...
// Render static widget HTML
function renderStaticWidget(widget: any, posts: any[], viewMode: string = 'all') {
  ...
  if (filteredPosts.length === 0) {
    return `
      <div style="text-align: center; padding: 48px 12px;">
        <div style="width: 96px; height: 96px; background: #f3f4f6; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px;">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="#9ca3af">
            <path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/>
          </svg>
        </div>
        <h3 style="font-size: 18px; font-weight: 500; color: #111827; margin: 0 0 8px;">
  ...
  return `
    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 4px; max-width: 600px; margin: 0 auto;">
      ${filteredPosts.map(post => {
        ...
        return `
          <div style="position: relative; ${aspectRatio === 'square' ? 'aspect-ratio: 1/1' : 
                     aspectRatio === 'portrait' ? 'aspect-ratio: 3/4' : 'aspect-ratio: 4/3'}; 
                     background: #f3f4f6; border-radius: 8px; overflow: hidden; cursor: pointer;">
            ${image ? `
            ...
            `}
            ${post.pinned ? `
              <div style="position: absolute; top: 4px; right: 4px; background: #000; color: #fff; border-radius: 4px; padding: 2px 6px; font-size: 10px; font-weight: 500;">
                PINNED
              </div>
            ` : ''}
            ${post.videos && post.videos.length > 0 ? `
              <div style="position: absolute; bottom: 4px; left: 4px; background: rgba(0,0,0,0.7); color: #fff; border-radius: 4px; padding: 2px 6px; font-size: 10px; font-weight: 500;">
                VIDEO
              </div>
            ` : ''}
          </div>
...
<head>
  ...
  <style>
    ...
    .post-card{position:relative;aspect-ratio:1/1;background:#f3f4f6;border-radius:8px;overflow:hidden;cursor:pointer}
...
<body>
  <div class="widget-container">
    ...
    <div class="view-toggle">
      <button class="view-btn ${viewMode === 'all' ? 'active' : ''}" onclick="window.location.href='?view=all'">All</button>
      <button class="view-btn ${viewMode === 'videos' ? 'active' : ''}" onclick="window.location.href='?view=videos'">Videos</button>
    </div>
    
    ${renderStaticWidget(widget, posts, viewMode)}
  </div>
</body>
</html>`;
    ...
    console.error('Error generating static embed:', error);
    const errorHtml = `
<!doctype html>
<html>
<head>
